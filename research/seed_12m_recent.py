"""Seed a 12-month selection pool for the 2023-2025 period (SP500 superset).

Mirrors seed_2m3m.py but with SEL_MONTHS=12 and monthly keys spanning the
2023-2025 walk-forward window (sel_starts ~2023-12-01 .. 2025-10-01). All
analyzed pairs are stored UNFILTERED (like sp500_12m.pkl); p-value / half-life
filtering happens at load time via the standard pool path.

The return-divergence (<=0.10) pre-filter is applied at seed time so we never
run cointegration on the full ~120k SP500 pairs per window.
"""
...
POOL_PATH = 'research/cache/sp500_12m_recent.pkl'
N_WORKERS = 8
THREADS_PER_WORKER = 1
DIVERGENCE = 0.10
SEL_MONTHS = 12

# Selection windows: monthly sel_start from 2023-12-01 to 2025-10-01.
WIN_START = date(2023, 12, 1)
WIN_STOP = date(2025, 10, 1)

# Price fetch range must cover the last selection window end (~2026-09-30).
FETCH_START = '2023-10-01'
FETCH_END = '2026-10-31'
...
def _process_impl(sel_start, sel_end):
    ...
    # Store all pairs unfiltered (filtering is done at load time).
    if len(df):
...
def main():
    parser = argparse.ArgumentParser(description='Seed 12-month SP500 pool for 2023-2025')
    ...
    vprint("Step 1/4: Fetching SP500 prices for 12m selection windows...")
    t0 = time.time()
    loader = DataLoader(TICKERS_SP500, start=FETCH_START, end=FETCH_END, use_warmup=True)
    ...
    all_windows = []
    curr = WIN_START
    stop = WIN_START + relativedelta(months=1) if args.test else WIN_STOP
    while True:
        sel_end = curr + relativedelta(months=SEL_MONTHS) - pd.Timedelta(days=1)
        if curr > stop:
            break
